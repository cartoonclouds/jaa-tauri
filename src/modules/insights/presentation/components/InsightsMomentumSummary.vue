<script setup lang="ts">
  ...
  /**
   * Defines insights momentum summary props.
   */
  interface InsightsMomentumSummaryProps {
    overview: IMetric[];
  }

  const props = defineProps<InsightsMomentumSummaryProps>();

  const metricViewsById = computed(() => {
    const result = new Map<string, InsightCardMetricDefinition>();
